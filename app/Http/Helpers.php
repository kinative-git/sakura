...
// discount format for math operation
if (! function_exists('formated_discounted_price')) {
    function formated_discounted_price($product,$variant)
    {
        $price = $variant->price;
        $tax = 0;
    ...
    function get_cached_products($category_id = null) {
        ...
        if ($category_id != null) {
            ...
            return Cache::remember('products', 86400, function () use ($products) {
                return $products->latest()->limit(15)->get();
            });
        }
    }
}
...
if (! function_exists('currency_symbol')) {
    function currency_symbol()
    {
        ...
        $code =Cache::remember('curency_code', 86400, function (){
            return \App\Currency::findOrFail(get_setting('system_default_currency'))->code;
        });


        if(Session::has('currency_code')){

            $currency =Cache::remember('curency', 86400, function (){
                return  Currency::where('code', Session::get('currency_code', $code))->first();
            });
        }
        else{
            $currency =Cache::remember('curency', 86400, function () use ($code){
