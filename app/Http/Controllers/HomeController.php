...
class HomeController extends Controller
{
    ...
    public function end_section_products(Request $request) {

        $products = Product::whereIn('id',json_decode(get_setting('slider_under_products')))->paginate(30);

        return view('frontend.seasonal_products', compact('products'));
    }
...
//        $categories = Category::where('level', 0)->orderBy('name', 'asc')->get();
    ...
    {
        $keywords = array();
        $products_key = Product::where('published', 1)->where('tags', 'like', '%'.$request->search.'%')->get();
        foreach ($products_key as $key => $product) {
        ...
        $products_q =  filter_products(Product::query());

        $products_name = $products_q->where('published', 1)
                        ->where(function ($q) use($request) {
                            $q->where('name', 'like', '%'.$request->search.'%');
                        })
                    ->get();
        $products=$products_key->merge($products_name);
...
//        $products = filter_products(Product::where('published', 1)->where('name', 'like', '%'.$request->search.'%'))->orWhere('tags', 'like', '%'.$request->search.'%')->get()->take(3);
    ...
    public function listingByCategory_offer(Request $request, $category_slug){
        $category = Category::where('slug', $category_slug)->first();
        if ($category != null) {
            return $this->search($request, $category->id,null,true);
        }
        abort(404);
    }
    ...
    public function search(Request $request, $category_id = null, $brand_id = null,$disc=null)
    {
        // dd($request);
        $query = $request->q;
...
//        }
    ...
    public function load_side_menue(Request $request){
        return view('frontend.partials.side_part_nav');
    }
}
