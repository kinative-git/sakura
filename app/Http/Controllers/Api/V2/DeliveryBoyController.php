...
//        return new DeliveryBoyPurchaseHistoryMiniCollection($order_query->where('assign_delivery_boy', $id)->latest('delivery_history_date')->paginate(10));
    ...
    public function colleection_received(Request $request,$id){
        $collection_query=DeliveryBoyCollection::where('user_id',$id);
        $from=$request->from_date;
        $to=$request->to_date;
        if($from && $to){
            // dd(Carbon::createFromFormat('Y-m-d',$from)->addDays(-1));
        ...
        return new DeliveryBoyCollectioCollection($collection_query->latest()->paginate(10));
    }
    ...
    public function earning_summary(Request $request,$id)
    {
        ...
        $yesterday_collection = DeliveryHistory::where('delivery_status', 'delivered')
            ...
            'this_week_earning'=>format_price($this_week_collection),
            'this_month_earning'=>format_price($this_month_collection),
            'total_earning' =>format_price($total),
            'from_date'=>$from,
            'to_date'=>$to,
    ...
    public function colleection_received_summary(Request $request,$id){
        ...
        return response()->json([
            ...
            'this_week_collection'=>format_price($this_week_collection),
            'this_month_collection'=>format_price($this_month_collection),
            'total_collection' =>format_price($total),
            'from_date'=>$from,
            'to_date'=>$to,
        ]);
    }
    ...
    public function earning_paid(Request $request,$id){
        ...
        $collection_query=DeliveryBoyPayment::where('user_id',$id);
        $from=$request->from_date;
        $to=$request->to_date;
        if($from && $to){
            // dd(Carbon::createFromFormat('Y-m-d',$from)->addDays(-1));
        ...
        return new DeliveryBoyPaymentCollection($collection_query->latest()->paginate(10));
