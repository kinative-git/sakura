...
class ProductController extends Controller
{
    ...
    public function shop_by_cats(Request $request){
        $result=[];
       $category_ids = json_decode(get_setting('product_yours_categories'));

       $temp=[];
       $temp['category']=array('id'=>0,'name'=>'all home','icon'=>static_asset('assets/img/all_icon.png'));
       $temp['products']=new ProductMiniCollection(\App\Product::where('published', '1')->where('featured', '1')->take(15)->get());
       ...
       foreach($category_ids as $cat_id){
           ...
           $temp2['id']=$cat->id;
           $temp2['name']=$cat->name;
           $temp2['icon']= api_asset($cat->icon);
           $temp['category']= $temp2;
           $temp['products']= new ProductMiniCollection(filter_products($products)->latest()->take(15)->get());
           array_push($result,$temp);
    ...
    public function cat_offers($id, Request $request){
        $category_ids = CategoryUtility::children_ids($id);
        $category_ids[] = $id;

        $products = Product::whereIn('category_id', $category_ids)->where('discount','>',0);
