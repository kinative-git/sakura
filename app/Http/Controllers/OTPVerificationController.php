...
class OTPVerificationController extends Controller
{
    ...
    public function resend_verificcation_code(Request $request){
        ...
        sendSMS($user->phone, env("APP_NAME"), 'Your Amicon OTP code is '.$user->verification_code);
    ...
    public function reset_password_with_code(Request $request){
        if (($user = User::where('phone', '+88'.$request->phone)->where('verification_code', $request->code)->first()) != null) {
    ...
    public function send_order_code($order){
        if(json_decode($order->shipping_address)->phone != null){
            sendSMS(json_decode($order->shipping_address)->phone, env('APP_NAME'), 'Your '.env('APP_NAME').' order has been placed and Order Code is : '.$order->code);
        }
    }
