                ...
                <table class="table aiz-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th data-breakpoints="lg">{{ translate('Mark As Non-Delivered')}}</th>
                            <th data-breakpoints="lg">{{ translate('Mark As Delivery-Cancel')}}</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    <label class="aiz-switch aiz-switch-success mb-0">
                                        ...
                                        <input onchange="update_status_non_delivered(this)" value="{{ $delivery->id }}" type="checkbox">
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td>
                                    <label class="aiz-switch aiz-switch-success mb-0">
                                        <input onchange="update_status_delivery_cancel(this)" value="{{ $delivery->id }}" type="checkbox">
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td class="text-right">
...
@section('script')
    <script type="text/javascript">
        ...
        function update_status_non_delivered(selectObject) {
            var order_id = selectObject.value;
            var status = "non_delivered";
            $.post('{{ route('orders.update_delivery_status') }}', {
                _token      :'{{ @csrf_token() }}',
                order_id    :order_id,
                status      :status
        ...
        function update_status_delivery_cancel(selectObject) {
            var order_id = selectObject.value;
            var status = "delivery_cancel";
            $.post('{{ route('orders.update_delivery_status') }}', {
                _token      :'{{ @csrf_token() }}',
                order_id    :order_id,
                status      :status
