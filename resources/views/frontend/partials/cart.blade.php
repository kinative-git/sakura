...
@endphp
<a href="javascript:void(0)" class="d-flex align-items-center text-primary h-100 cart-toggler cart-trigger bg-base-1 rounded-left text-center px-3 z-1021"  type="button" data-toggle="class-toggle" data-target=".cart-sidebar"  data-toggle="dropdown" data-display="static">
    <span class="position-relative text-reset btn bg-sm btn-circle btn-icon">
        <i class="la la-shopping-cart  opacity-100" style="font-size: 30px;"></i>
        <span class="absolute-top-right" style="top: 3px;right: -5px;">
            @if(isset($cart) && count($cart) > 0)
                <span class="badge badge-inline badge-pill text-dark cart-count shadow-md" style="width: 16px;height: 16px;font-size: 8px;background-color:white;">{{ count($cart)}}</span>
            @else
