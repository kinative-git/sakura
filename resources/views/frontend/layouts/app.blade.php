...
<script>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $.post('{{ route('home.section.side_menue') }}', {_token:'{{ csrf_token() }}'}, function(data){
                $('#sid-men').html(data);
            ...
            $('.side-menu [data-id]').each(function(i, el) {
                ...
                $(el).on('click',function(){
                    $('.side-menu').addClass('slide');
                    $('#cat-menu-'+id).addClass('slide');
                });
            });
            $('.sub-menu-wrap .back-to-menu').on('click',function(){
                $('.side-menu').removeClass('slide');
                $('.sub-menu-wrap').find('.slide').removeClass('slide');
            });
        ...
        $('#search_two').on('keyup', function(){
            search2();
        });

        $('#search_two').on('focus', function(){
            search2();
        });
        function search(){
        ...
        function search2(){
            var searchKey = $('#search_two').val();
            if(searchKey.length >= 3){
                $('body').addClass("typed-search-box-shown");

                $('.typed-search-box-two').removeClass('d-none');
                $('.search-preloader').removeClass('d-none');
                $.post('https://kholabazaar.com.bd/ajax-search', { _token: AIZ.data.csrf, search:searchKey}, function(data){
                    if(data == '0'){
                        // $('.typed-search-box').addClass('d-none');
                        $('#search-content-two').html(null);
                        $('.typed-search-box-two .search-nothing').removeClass('d-none').html('Sorry, nothing found for <strong>"'+searchKey+'"</strong>');
                        $('.search-preloader').addClass('d-none');

                    }
                    else{
                        $('.typed-search-box-two .search-nothing').addClass('d-none').html(null);
                        $('#search-content-two').html(data);
                        $('.search-preloader').addClass('d-none');
                    }
                });
            }
            else {
                $('.typed-search-box-two').addClass('d-none');
                $('body').removeClass("typed-search-box-shown");
            }
        }
            ...
            if($('#option-choice-form input[name=quantity]').val() > 0 && checkAddToCartValidity()){
                $.ajax({
                   ...
                   success: function(data){
                       ...
                       if(  $('#available-quantity').classList.contains('text-danger')){
                            $('#available-quantity').removeClass('text-danger');
                       }
                       var txt=`${data.quantity} available`;
                       if(data.quantity==0){
                        $('#available-quantity').addClass('text-danger');
                        txt=`Out of Stock`;
                       }

                       $('#available-quantity').html(txt);
